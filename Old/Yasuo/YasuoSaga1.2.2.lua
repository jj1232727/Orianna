...
CastQ = function(target)
    if  Game.CanUseSpell(0) == 0 and target.pos:DistanceTo() <= 450 and GotBuff(myHero, "YasuoQ3W") == 0 and (Game.Timer() - OnWaypoint(target).time < 0.15 or Game.Timer() - OnWaypoint(target).time > 1.0) then -- 3 check
        local aim = GetPred(target,1125, .25 + Game.Latency())    
        --self:CastTiamat()
            if myHero.pathing.isDashing and target.pos:DistanceTo() <= 375  then
                Control.CastSpell(HK_Q)
            else
                CastSpell(HK_Q, target)--(HK_Q, target,pos, math.huge, Q1 * 1000)
            end
    end
...
CastE = function(target)
    if Saga.Combo.UseE:Value() and target.pos:DistanceTo() <= 475 and Game.CanUseSpell(2) == 0 and GotBuff(target, "YasuoDashWrapper") == 0 and not UnderTurret(DashEndPos(target)) then  
...
getPath = function(focus, minion)
    ...
    if isOnSegment  and GetDistanceSqr(minion.pos, pointSegment) < 150 then
        return true
    end
    return false
end
...
function()
	...
	MenuElement({ id = "blank", type = SPACE ,name = "Version BETA 1.2.2"})
	--Combo
