...
local isEvading = ExtLibEvade and ExtLibEvade.Evading
	local validTarget,
        ...
        SpellData = {
            ...
            ["Jinx"] = {
                ["jinxwmissile"] = {name = "Zap!", radius =  50},
                ["jinxr"] = { name = "Super Mega Death Rocket!", Width =  50}
            },
            ["Jhin"] = {
            ...
            ["Yasuo"] = {
                ["yasuoq3w"] = { name = "Gathering Storm", radius =  50}
            },
            ["Zed"] = {
...
function CalcPhysicalDamage(source, target, amount)
  ...
  PassivePercentMod = function(source, target, amount, damageType)
    ...
    --[[if self:GetTiamat() > -1 and self.menu_ComboTiamat then
        basedamage = dmg + aa * 0.7
    end]]--
    --[[if self:GetIgniteIndex() > -1 and CanCast(self:GetIgniteIndex()) then
        basedamage = basedamage + 50 + 20 * GetLevel(myHero.Addr) / 5 * 3
    end]]--
...
AutoR = function()
    ...
    local target = GetTarget(1400)
        --forloop
        if Saga.Rset.UseRA:Value() and Game.CanUseSpell(3) == 0 and number >= Saga.Rset.RCount:Value() and not UnderTurret(target) and Saga.Combo.UseR:Value() then
            CastSpell(HK_R, p)
        else 
            if target and Saga.Rset.UseR:Value() and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(3) == 0 then
    ...
    for i = 1, Game.MissileCount() do
        ...
        if obj then
            ...
            if  target and table.contains(walllist, obj.missileData.name:lower()) and  Game.CanUseSpell(1) == 0 and isOnSegment and (GetDistanceSqr(castp, linesegment) <= 300  * 300)then --
                ...
                Control.CastSpell(HK_W, obj.missileData.startPos)
                end
            end
...
CastQ = function(target)
    if  Game.CanUseSpell(0) == 0 and target.pos:DistanceTo() <= 450 and GotBuff(myHero, "YasuoQ3W") == 0 and (Game.Timer() - OnWaypoint(target).time < 0.15 or Game.Timer() - OnWaypoint(target).time > 1.0) then -- 3 check
        local aim = GetPred(target,math.huge, .25 + Game.Latency())    
        --self:CastTiamat()
            if myHero.pathing.isDashing and target.pos:DistanceTo() <= 220  then
                CastSpell(HK_Q, aim, math.huge, Q1 * 1000)
            else
                if target.pos:DistanceTo() <= 450 then
                CastSpell(HK_Q, aim, math.huge, Q1 * 1000)
                end
            end 
    end
    ...
    if Game.CanUseSpell(0) == 0 and target.pos:DistanceTo() <= 900 and GotBuff(myHero, "YasuoQ3W") == 1 and (Game.Timer() - OnWaypoint(target).time < 0.15 or Game.Timer() - OnWaypoint(target).time > 1.0) then -- is 3 check
        local aim = GetPred(target,math.huge, .25 + Game.Latency())   
        if myHero.pathing.isDashing and target.pos:DistanceTo() <= 220 then
            CastSpell(HK_Q, aim, math.huge, Q2 * 1000)
        else
            CastSpell(HK_Q, aim, math.huge, Q2 * 1000)
        end 
    end 
...
CastE = function(target)
    if Saga.Combo.UseE:Value() and target.pos:DistanceTo() <= 475 and Game.CanUseSpell(2) == 0 and GotBuff(target, "YasuoDashWrapper") == 0 and not UnderTurret(target) then  
        Control.CastSpell(HK_E, target)
    else
        if Saga.Combo.UseE:Value() and GotBuff(target, "YasuoDashWrapper") == 0 and target.pos:DistanceTo() <= 475 and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(2) == 0  then
            Control.CastSpell(HK_E, target)
        end
...
getPath = function(focus, minion)
    ...
    if isOnSegment  and GetDistanceSqr(minion.pos, pointSegment) < 80 then
        print("Found Path")
        return true
    end
    return false
...
CastR = function(target) 
        ...
        --forloop
        if Game.CanUseSpell(3) == 0 and number >= Saga.Rset.RCount:Value() and not UnderTurret(target) and Saga.Combo.UseR:Value() then
            CastSpell(HK_R, p)
        else 
            if Saga.Combo.UseR:Value() and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(3) == 0 then
                CastSpell(HK_R, target)
            end 
        end
end
    ...
    for i = 1, SagaMCount() do 
        ...
        if Game.CanUseSpell(0) == 0 and Saga.Clear.UseQ:Value() then
            ...
            if QDmg > minion.health and minion.pos:DistanceTo() < 400 then
                CastSpell(HK_Q, minion, math.huge, Q1 * 1000)
            end
        end
        if Game.CanUseSpell(2) == 0 and minion and Saga.Combo.UseE:Value() then
            ...
            if GotBuff(minion, "YasuoDashWrapper") == 0 and not UnderTurret(minion) and  minion.pos:DistanceTo() < 475 then
...
LastHit = function()
    for i = 1, SagaMCount() do 
        ...
        if Game.CanUseSpell(0) == 0 and Saga.Lasthit.UseQ:Value() then
            local QDmg =  CalcPhysicalDamage(myHero,minion, (myHero:GetSpellData(_Q).level* 25 - 5) + (myHero.totalDamage)  + myHero.totalDamage)
            if QDmg > minion.health and minion.pos:DistanceTo() < 400 then
                CastSpell(HK_Q, minion, math.huge, Q1 * 1000 * 1000)
            end
        end
    end
end
...
Saga_Menu = 
function()
	Saga = MenuElement({type = MENU, id = "Irelia", name = "Saga's Yasuo: Uindo Bitchi", icon = AIOIcon})
